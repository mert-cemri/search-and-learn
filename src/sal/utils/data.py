...
def save_dataset(dataset, config):
    if config.push_to_hub:
        ...
        if config.max_tokens != 2048:
            dataset.to_json(f"{config.output_dir}/{model_path}-{target_model_path}_n{config.n}_b{config.rm_regularizer}_q{config.dataset_end}_period{config.period}_t{config.max_tokens}_num_iters{config.num_iterations}_model_len{config.max_model_len}.jsonl", lines=True)
            logger.info(f"Saved completions to {config.output_dir}/{model_path}-{target_model_path}_n{config.n}_b{config.rm_regularizer}_q{config.dataset_end}_period{config.period}_t{config.max_tokens}_num_iters{config.num_iterations}_model_len{config.max_model_len}.jsonl")
        else:
            dataset.to_json(f"{config.output_dir}/{model_path}-{target_model_path}_n{config.n}_b{config.rm_regularizer}_q{config.dataset_end}_period{config.period}_model_len{config.max_model_len}.jsonl", lines=True)
            logger.info(f"Saved completions to {config.output_dir}/{model_path}-{target_model_path}_n{config.n}_b{config.rm_regularizer}_q{config.dataset_end}_period{config.period}_model_len{config.max_model_len}.jsonl")
